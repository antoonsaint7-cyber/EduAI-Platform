...
function setup(user = { id: 'teacher-1', tenant_id: 'tenant-1', role: 'teacher' }, rowsByCall = []) {
  let handler;
  const app = { patch(_path, _auth, fn) { handler = fn; } };
  let index = 0;
...
test('RAG scope rejects lesson without course', async () => {
  const handler = setup();
  const res = response();
  await handler({ params: { id: '11111111-1111-1111-1111-111111111111' }, body: { lessonId: '22222222-2222-2222-2222-222222222222' } }, res, assert.fail);
  assert.equal(res.statusCode, 400);
});

test('RAG scope rejects teacher-owned course mismatch', async () => {
  const handler = setup({ id: 'teacher-1', tenant_id: 'tenant-1', role: 'teacher' }, [[]]);
  const res = response();
  await handler({ params: { id: '11111111-1111-1111-1111-111111111111' }, body: { courseId: '22222222-2222-2222-2222-222222222222' } }, res, assert.fail);
...
test('RAG scope updates a document after ownership checks', async () => {
  ...
  const handler = setup({ id: 'teacher-1', tenant_id: 'tenant-1', role: 'teacher' }, [[{ id: '22222222-2222-2222-2222-222222222222' }], [document]]);
  const res = response();
  await handler({ params: { id: document.id }, body: { courseId: document.course_id } }, res, assert.fail);
