...
function setup({ client = null, rows = [] } = {}) {
  ...
  return async (req, res) => auth(req, res, () => handler(req, res, assert.fail));
}
...
test('grounded tutor returns a safe no-source response', async () => {
  ...
  const res = response();
  await handler({ body: { message: 'ما هو الكسوف؟' } }, res);
...
test('grounded tutor uses retrieved lesson/document context and reports missing AI configuration', async () => {
  ...
  const res = response();
  await handler({ body: { message: 'متى يحدث الكسوف؟', referenceText: 'الكسوف يحدث عندما يحجب القمر ضوء الشمس عن جزء من الأرض.', referenceTitle: 'درس الفلك' } }, res);
...
test('grounded tutor calls the OpenAI chat completion with grounded sources', async () => {
  ...
  const res = response();
  await handler({ body: { message: 'متى يحدث الكسوف؟' } }, res);
