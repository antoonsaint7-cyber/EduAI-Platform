...
function createDocumentExtractor({ adapters = {} } = {}) {
  return async function extract(filePath, mimeType) {
    ...
    if (typeof adapter !== 'function') throw new Error(`No extraction adapter configured for ${mimeType}.`);
    const result = await adapter(filePath);
    if (!result || typeof result.text !== 'string' || !result.text.trim()) throw new Error('Extraction adapter returned no text.');
    return {
