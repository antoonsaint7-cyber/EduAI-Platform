...
export function analyzeSource({ text, metadata = {} }) {
  ...
  return {
    type: 'curriculum-analysis',
    metadata,
    textLength: text.length,
    sections: headings.map((title, index) => ({ id: `section-${index + 1}`, title })),
    sourceHashInput: text
  };
}
...
export function generateDraft({ analysis, sourceText, subject = '', grade = '' }) {
  ...
  const curriculum = {
    ...
    title: analysis.metadata.title || 'Generated curriculum',
    grade,
    subject,
    sourceVersion: analysis.metadata.sourceVersion || 'uploaded-source',
    units: [{
      id: 'unit-1',
      title: analysis.metadata.unitTitle || 'Unit 1',
      lessons: sections.map((section) => ({
        ...
        generated: {
          ...
          explanation: 'Draft generated from the supplied source. AI generation should replace this placeholder.',
          examples: [],
          questions: []
        },
        evaluation: { status: 'pending', grounded: false, issues: ['AI generation not yet connected'] }
      }))
    }]
  };
