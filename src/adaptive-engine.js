...
function rankNextQuestions(questions = [], profile = []) {
  ...
  return (Array.isArray(questions) ? questions : []).map(q => {
    const p = bySkill.get(q.skill); const mastery = p?.mastery ?? 0; const difficulty = clamp(q.difficulty ?? 0.5 * 100);
    const target = 45 + mastery * 0.45;
    const fit = 100 - Math.abs(difficulty - target);
    const weakness = 100 - mastery;
    return { ...q, adaptive_score: Math.round((fit * 0.55 + weakness * 0.45) * 100) / 100 };
