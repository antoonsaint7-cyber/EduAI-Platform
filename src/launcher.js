...
app.get('/ready', async (_req, res) => {
  ...
  try {
    ...
    const redis = getRedis(); if (!redis) throw new Error('REDIS_URL is not configured.');
    await redis.ping(); checks.redis = true;
    res.status(200).json({ status: 'ready', checks });
  } catch (error) { res.status(503).json({ status: 'not_ready', checks, error: error?.message || 'readiness check failed' }); }
});
...
server.on('error', (error) => { console.error(`HTTP server error: ${error?.message || error}`); process.exitCode = 1; });
server.listen(port, '0.0.0.0', () => { console.log(`EduAI Platform running on port ${port}`); });
...
async function shutdown(signal) {
  ...
  server.close(async () => {
    try { await close(); await closeRedis(); clearTimeout(forceExit); process.exit(0); }
    catch (error) { console.error(error?.message || error); clearTimeout(forceExit); process.exit(1); }
  });
}
